...
func (gb *GroupBase) URLTest(ctx context.Context, url string, expectedStatus utils.IntRanges[uint16]) (map[string]uint16, error) {
	...
	for _, proxy := range proxies {
		...
		go func(proxy C.Proxy) {
			...
			// 终极补丁：为整组测速中的节点强制延长时间
			testCtx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
			defer cancel()

			XDebugLog("[XHTTP-DEBUG] Group testing proxy starting: %s", proxy.Name())
			delay, err := proxy.URLTest(testCtx, url, expectedStatus)
			if err != nil {
				XDebugLog("[XHTTP-DEBUG] Group test failed for %s: %v", proxy.Name(), err)
			} else {
				XDebugLog("[XHTTP-DEBUG] Group test success for %s: %d ms", proxy.Name(), delay)
				lock.Lock()
				mp[proxy.Name()] = delay
				lock.Unlock()
			}
